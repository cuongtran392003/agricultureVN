...
export const ManagePlotScreen = ({
    ...
    const [farmId, setFarmId] = useState(""); // Thêm state quản lý farmId
    ...
    const handleSave = () => {
        ...
        if (!farmId) {
            Alert.alert("Thiếu thông tin", "Cô/chú vui lòng chọn Nông trại quản lý Lô đất này nhé!");
            return;
        }
    ...
    // Nút thêm lô đất (Mình đổi UI một chút cho giống với trang Nông trại cho đồng bộ)
    const AddPlotBtn = (
        <TouchableOpacity
            ...
            >
                {plots?.length === 0 ? (
                    <Text className="text-center text-gray-500 mt-10 text-[16px]">
                        Chưa có lô đất nào. Hãy thêm mới nhé!
                    </Text>
                ) : (
                    plots?.map((plot: Plot, index: number) => (
                        <Animated.View
                            key={plot._id}
                        ...
                        >
                            <View className="flex-row items-start mb-4">
                                <View className="w-[50px] h-[50px] rounded-full items-center justify-center bg-[#FEF3C7]">
                                    <FontAwesome5
                                        name="layer-group" // Đổi icon cho Lô đất để phân biệt với Farm
                                        size={20}
                                ...
                                <View className="flex-1 ml-3">
                                    ...
                                    {/* Hiển thị tên Nông trại quản lý Lô đất này */}
                                    {plot.farmId?.name && (
